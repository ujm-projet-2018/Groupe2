...
complex* analyse(short* amplitudes, int depart, int N);
complex* recupere_point_complex(short* amplitudes, int depart, int n);
...
void usage(char* s){   /* explique le fonctionnement du programme */
  ...
  fprintf(stderr,"      -n \"entier\" > 1 : pour definir la fenetre de l'analyse de fourier(nombre de point)\n");
  fprintf(stderr,"      -0 \"entier\" > 1 : pour definir le point de depart de l'analyse de fourier\n");
...
int main(int argc, char** argv){
    ...
    int l = 1200, h = 750, echan_fourier = 0, filtre = 1, debug = 1, depart = 0;   // les options
    ...
    while ((op = getopt(argc, argv, "n:0:f:l:h:p:ud")) != -1){   /* cherche les options sur la ligne de commande */
      ...
      case 'u':   /* affiche l'usage du programme */
        usage(argv[0]);
        exit(0);
      case 'n':    /* permet de preciser la valeur de l'echantillonage */
        echan_fourier = atoi(optarg);  /* recupere la valeur situer juste apres l'option */
        break;
      case 'f':     /* precise le fichier a utiliser */
        nomFich = optarg;
        break;
      case '0':     /* point de depart de l'analyse */
        depart = atoi(optarg);
        break;
    ...
    // defini le nombre de point pour la FFT
    if (echan_fourier == 0)
         echan_fourier = nb_point;
    ...
    fft = analyse(amplitudes, depart, log2(echan_fourier));
    
    tracer_spectre(fft, clicx, clicy, pow(2, log2(echan_fourier)), dx, zoom, dec_x, dec_y, echelley, freqEch, l, h);
...
complex* analyse(short* amplitudes, int depart, int q){
     int e, N = pow(2, q);
     complex* echange;
     complex* freq;
     complex* y = recupere_point_complex(NULL, 0, N);
     
     freq = recupere_point_complex(amplitudes, depart, N);
...
void tracer_spectre(complex* fft, int clicx, int clicy, int n, double dx, double zoomx, int dec_x, int dec_y, double echelley, int freqEch, int l, int h){
     ...
     for (i = 0; i<n-1; i++){
          tracerSegment(dx, l/2, dec_y, zoomx, x, y, i+1, cabs(fft[i+1])*echelley/max, l, h);
          x = (i+1);
